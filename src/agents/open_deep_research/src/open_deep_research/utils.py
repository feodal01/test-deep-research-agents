...
async def tavily_search(
    ...
    # Initialize summarization model with retry logic (with per-node overrides)
    model_api_key = get_api_key_for_model(configurable.summarization_model, config)
    sum_overrides = get_node_model_overrides("summarization", configurable.summarization_model, config)
    summarization_model = init_chat_model(
        model=sum_overrides.get("model", configurable.summarization_model),
        max_tokens=configurable.summarization_model_max_tokens,
        api_key=sum_overrides.get("api_key", model_api_key),
        base_url=sum_overrides.get("base_url"),
...
async def tavily_search_async(
    ...
    """
    # Initialize the Tavily client with API key and optional base URL from config
    api_key = get_tavily_api_key(config)
    base_url = get_tavily_base_url(config)
    try:
        tavily_client = AsyncTavilyClient(api_key=api_key, api_base_url=base_url)
    except TypeError:
        # Older tavily-python version without api_base_url support
...
def get_node_model_overrides(node: str, default_model_name: str, config: RunnableConfig) -> dict:
    """Return per-node model overrides from environment or config.

    Priority:
    1) If GET_MODEL_OVERRIDES_FROM_CONFIG=true, read from config["configurable"]["modelOverrides"][node]
    2) Else read from environment variables: {NODE}_MODEL, {NODE}_BASE_URL, {NODE}_API_KEY

    Args:
        node: logical node name, e.g. "research", "summarization", "compression", "final_report"
        default_model_name: current configured model name for the node
        config: runtime runnable config
    """
    should_get_from_config = os.getenv("GET_MODEL_OVERRIDES_FROM_CONFIG", "false").lower() == "true"
    overrides: dict = {}
    if should_get_from_config:
        model_overrides = (config.get("configurable", {}) or {}).get("modelOverrides", {}) or {}
        node_overrides = model_overrides.get(node, {}) or {}
        for key in ("model", "base_url", "api_key"):
            if key in node_overrides and node_overrides[key]:
                overrides[key] = node_overrides[key]
        return overrides
    # Environment fallback
    prefix = node.upper()
    env_model = os.getenv(f"{prefix}_MODEL")
    env_base_url = os.getenv(f"{prefix}_BASE_URL")
    env_api_key = os.getenv(f"{prefix}_API_KEY")
    if env_model:
...
def get_tavily_base_url(config: RunnableConfig):
    """Get Tavily base URL from environment or config.

    Supports both keys for compatibility:
    - Config: configurable.apiKeys.TAVILY_API_BASE_URL or configurable.baseUrls.TAVILY_BASE_URL
    - Env: TAVILY_API_BASE_URL or TAVILY_BASE_URL
    """
    should_get_from_config = os.getenv("GET_API_KEYS_FROM_CONFIG", "false")
    if should_get_from_config.lower() == "true":
        cfg = (config.get("configurable", {}) or {})
        api_keys = cfg.get("apiKeys", {}) or {}
        if api_keys.get("TAVILY_API_BASE_URL"):
            return api_keys.get("TAVILY_API_BASE_URL")
        base_urls = cfg.get("baseUrls", {}) or {}
        if base_urls.get("TAVILY_BASE_URL"):
            return base_urls.get("TAVILY_BASE_URL")
        return None
    # Env fallback
    return os.getenv("TAVILY_API_BASE_URL") or os.getenv("TAVILY_BASE_URL")
